...
RO-Crate \cite{Soiland-Reyes 2022} (section \ref{packaging-research-artefacts-with-ro-crate}) is a lightweight method to package research outputs along with
...
inheritance) in
Workflow
Testing\footnote\url{https://crs4.github.io/life\_monitor/workflow\_testing\_ro\_crate} 
...
used by the LifeMonitor\footnote\url{https://www.lifemonitor.eu/} service to
...
\begin{figure}
  \centering
      \includegraphics[width=0.3\textwidth]{figures/ch04/signposting.pdf}
      \caption[FAIR Signposting]{~\textbf{FAIR Signposting}.
  FAIR Signposting on a workflow PID \cite{Bayyarri 2022} 
  discovered from HTTP \texttt{Link:} headers using the
  Signposting tool\footnote{\url{https://pypi.org/project/signposting/}} shows
  ...
  JSON-LD\footnote{\url{https://workflowhub.eu/workflows/255.jsonld}} from
  workflowhub.eu follows the BioSchemas
  ComputationalWorkflow
  profile \footnote{\url{https://bioschemas.org/profiles/ComputationalWorkflow/1.0-RELEASE}} 
  to give workflow details not included in DataCite's general 
  JSON-LD\footnote{\url{https://data.crosscite.org/application/ld+json/10.48546/workflowhub.workflow.255.1}}.
}
