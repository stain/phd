...
{[}9--13{]} is transforming and will radically further transform the
range of scientific research opportunities and questions that can be
addressed {[}14,15{]}. Scientific conclusions and policy decisions
...
computable form {[}18{]}. Tools of particular interest span the fields
...
{[}26{]} are packaged into canonical workflow component libraries
{[}27{]}, rendering them reusable, and interoperable with one another.
...
\begin{itemize}
\item
  \textbf{computable Digital Specimen (DS) objects} {[}30{]} from DISSCo
  for the scientific input/output data that can be consumed/produced by
  workflows and other applications.
\item
  \textbf{workflow objects, implemented as RO-Crates} {[}23{]}, from
...
system {[}20{]} in conjunction with Common Workflow Language (CWL)
{[}21{]} has been chosen. CWL is a workflow specification standard
...
tool components, Galaxy {[}20{]} supports multiple domains. Workflows
...
{[}27{]}. The WorkflowHub, which facilitates CWL and enables workflows
...
digital object methods {[}44--46{]} for the interactions between
...
Object {[}29{]} and is defined formally as ``a unit composed of data
...
comprehensive manner {[}27{]}. Each FDO is anchored by a globally unique
...
openDS {[}47{]} is the developing specification for open Digital
Specimens and other related object types, defining: i) the logical
structure and content of Digital Specimen (DS), Basic Image Object (BIO)
and other object types, and the operations permitted on them; ii) the
handling rules and behaviors governing digital specimen object
operations in general; and iii) serialization and packaging as
...
`workflow components' {[}48{]}. openDS is essential to future FAIR
...
(DiSSCo) {[}6, 25, 30{]} European research infrastructure.

Henceforth we refer to these as openDS FDOs.

\hypertarget{fair-packaging-of-researchworkflow-objects-with-ro-crate}{%
...
the work reproducible. Research objects {[}49{]} are a general approach
...
uses JSON-LD {[}50{]} with \href{https://schema.org/}{Schema.org}
{[}51{]} for its common metadata representation. It is extensible with
domain-specific vocabularies in a growing range of specializing RO-Crate
profiles, e.g., for domains such as earth sciences {[}52{]}, biosciences
{[}53{]}; for object types such as data or workflow {[}54{]}; or for
...
the FDO Metadata objects foreseen by the FDO Framework {[}53,31{]}.
Combined with FAIR Signposting {[}55{]} for resolving persistent
...
\hypertarget{automating-digitization-and-capturing-the-process}{%
...
information and attached labels {[}56{]}. The practices of different
...
expected through the planned DiSSCo research infrastructure {[}6, 25,
30{]}.
...
transcribed data {[}59{]}. For microscope slide and pinned insect
...
\item
  ...
  and needs as outlined in the DiSSCo Data Management Plan {[}28{]}
...
\item
  ...
  work. These kinds of Research Objects {[}49{]} are an established
...
{[}27{]}. Each FDO type originates from different Research
...
{[}66{]}, JSON-LD and Schema.org {[}49{]}, and community-accepted
