...
More recently, interoperability have been partially addressed by Common Workflow Language Prov (CWLProv)~\cite{Khan 2019}, which represents workflow enactments as \acrshortpl{RO} serialised according to the Big Data Bag (BDBag) approach~\cite{Chard 2016}.
...
%Thus, users of a WfMS that implements this profile (or Provenance Run Crate, which inherits it) are able to register their workflows in WorkflowHub -- together with an execution trace -- by simply running them and uploading the resulting RO-Crates.
...
We extract the prospective provenance contained in Galaxy's \acrshort{YAML}-based 
...
The \footurl{https://github.com/alpha-unito/streamflow}{StreamFlow} framework~\cite{Colonnelli 2021} is a container-native \acrshort{WfMS} based on the \acrshort{CWL} standard.
...
Recent releases of Autosubmit 4 include an updated \acrshort{YAML} configuration management system that allows users to combine multiple YAML files into a single unified configuration file.
