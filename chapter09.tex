...
Another aspect that is not highlighted well in ontologies is where to stop formalization of an object. In schema.org, many properties like \texttt{licence} are defined as having a type of \footurl{http://schema.org/URL}{\texttt{URL}}, hinting that the node is not expected to be explained with further properties, but that the attribute's primary purpose is navigation or identification. 

This would be a key aspect of Linked Data, which traditionally have had the undefined structure of ``follow your nose'' navigation -- a client may attempt to request any node identifier (if it's a URL), and if, with content-negotiation, it returns some RDF, then that could be integrated into the knowlege graph, hopefully adding more description of that node, although possibly using other vocabularies. However, ontologies have not commonly indicated navigation waypoints as done in Schema.org.
...
\begin{itemize}
    \item Simplify user interfaces, e.g. a display name and type before the user chooses to click the link
    \item Vocabulary adaptation, for instance describing with schema.org in the crate, what is expressed in FOAF vocabulary at the URL.
    \item Unify descriptions of semantic artefacts and web pages. Making ``ad-hoc'' semantic artefacts within the crate.
    \item Archive ``true at time of writing'' descriptions for longevity. An RO-Crate is self-contained and this reduces service dependencies (at the risk of outdated information).
\end{itemize}
...
In this way we see a breaking of the chicken-and-egg problem, for instance orcid.org recently added schema.org content negotiation, but \emph{after} RO-Crate started describing people using ORCID identifiers\footnote{My earlier contribution to ORCID had in fact already established content-negotiation to RDF, but with the classical FOAF vocabulary. The registry is currently returning person description with different semantic models depending on which serialization is requested}. However when reflecting on the propositions of FAIR Digital Objects as covered in \ref{chapter:fdo}, we can see a problem if every reference from an FDO must go to another pre-existing FDO (or at least a registered PID), in that there must then be a a linear order of creation within an FDO ecosystem, if it could not utilise the estabished human-readable Web as its bootstrapping ground. This could mean large cross-organizational delays with a stronger need for collaboration, or alternatively, starting with a small metamodel that can gradually evolve to add more navigation when and if registries appear with FDO interfaces.
