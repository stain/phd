...
established {[}1--3{]}, and adaptation of \emph{workflow management
systems} (\textbf{WfMS}) together with \emph{software packaging and
containers} {[}4{]} have been proposed as key ingredients for making
research software usage \emph{FAIR} and reproducible {[}5--7{]}.
Recently it is also argued that computational workflows should also be
treated as FAIR Digital Objects {[}8{]} in their own right, with
identifier, metadata {[}2{]} and interoperability requirements {[}9{]}.
...
Building Blocks} (\textbf{BioBB}) {[}10{]} have been created as portable
...
library {[}10{]}, created and implemented within the BioExcel CoE, is a
...
principles for research software development best practices {[}7{]}.
...
from EDAM ontology {[}11{]}), \emph{example} files (essential for
...
workflows} {[}12{]}. Here, each workflow introduces individual building
blocks as needed to explain a particular scientific computational
method. We primarily expose the workflows as Jupyter Notebooks {[}13{]},
which has been highlighted as a valuable tool for reproducible
scientific workflows {[}14{]}. This offers a graphical interactive
...
\href{https://hub-bioexcel-binder.tsi.ebi.ac.uk/}{Binder} {[}15{]}
hosting, these workflows are reproducible across platforms, assisted by
BioConda {[}16{]} packaging of the building blocks and their software
dependencies.
...
infrastructure of the BioExcel Cloud portal {[}17{]} to give users a
...
(Galaxy {[}18{]}, CWL {[}19{]}, PyCOMPSs {[}20{]}) or manually generated
bindings (KNIME {[}21{]}). Corresponding workflows for the different
WfMS can be found in
\href{https://workflowhub.eu/collections/3}{WorkflowHub} {[}22--26{]}
...
{[}22{]}, KNIME {[}23{]}, CWL {[}24{]}, Jupyter Notebook {[}25{]},
PyCOMPSs {[}26{]}.'' \textgreater\}\}
...
{[}27{]} for bioinformatics and similarly
\href{https://en.wikipedia.org/wiki/CaBIG}{caBIG} {[}28{]} for cancer
...
The notion of \emph{abstract workflows} {[}29{]}, structural workflow
...
\emph{common motifs} for workflow operations {[}30{]} (e.g.~Data
...
components} had prototypical building blocks {[}31{]} which were
...
The \href{https://toolshed.g2.bx.psu.edu/}{Galaxy toolshed} {[}32{]} is
likewise populated with different sets of tool wrappers that are largely
made to be interoperable within a category.

The \emph{Common Workflow Language} (\textbf{CWL}) {[}19{]} has a strong
...
{[}33{]}. However workflow engines have multiple dependencies and often
...
{[}34{]}, with the option of abstract CWL for describing the canonical
workflow template, along with direct references to the workflow's GitHub
repository.

For instance in the RO-Crate for
\url{https://doi.org/10.48546/workflowhub.workflow.200.1} {[}26{]},
which can also be
...
*\href{https://rawcdn.githack.com/bioexcel/biobb_hpc_workflows/53958e7c278e53c277a7217057b785482f193f7f/ro-crate-preview.html\#workflows/MD/md_list.py}{main
...
{[}4{]}, particularly for Findability and Accessibility, as we can
...
\emph{application suite} (bio.tools entry {[}35{]}, homepage,
...
order for a Canonical Workflow {[}36{]} to follow and support FAIR
...
Canonical Workflow Frameworks for Research {[}36{]}.
...
applies {[}37{]} (e.g.~easy to install, documented, parallelizable,
...
which should have a corresponding persistent identifier (PID) {[}38{]}.
...
adapt the software to be workflow-ready {[}37{]} before using them as
