...
\emph{Computational Workflows} (or Scientific Workflows) are used to structure and automate a data analysis pipeline so that it can be scalable, portable and explainable \cite{Atkinson 2017,Leipzig 2021}. There exists a plethora of workflow systems and languages \cite{ch8-43}, although recent community effort have specified the Common Workflow Language \cite{Crusoe 2022} as a standard, particularly supporting the general use case of composing command line tools that read and write files. 

Nevertheless, workflow definitions come in multiple languages and file formats, and in varying degree list their dependencies and runtime requirements (such as software containers). 
