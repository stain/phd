...
\begin{landscape}
  ...
  \textbf{Encapsulation}: \emph{separate interface from implementation. Specify interface as contract, multiple implementations possible}
    & Indirection by PID gives separation. FDO principles are protocol independent, although it may be unclear which protocol to use for which FDO (although \texttt{0.DOIP/Transport} can be specified after already contacting DOIP). Cordra supports \footurl{https://www.cordra.org/documentation/api/doip.html}{native DOIP} \cite{CNRI 2023b}, \footurl{https://www.cordra.org/documentation/api/doip-api-for-http-clients.html}{DOIP over HTTP} \cite{CNRI 2023a} and \footurl{https://www.cordra.org/documentation/api/rest-api.html}{Cordra REST API}
...
RDA-A1-03D
  ...
  & Resolvable HTTP/HTTPS URIs are most common, now infrequent \acrshortpl{URN} are not directly resolvable \\
RDA-A1-04M
...
\cite{Kurowski 2021} details\footnote{
  \acrshort{EOSC-IF} has since been expanded on by a draft EOSC report \cite{Åkerström 2024}, which references the preprint of this section \cite{Soiland-Reyes 2024b}.
}
